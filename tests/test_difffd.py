...
class TestDiffFD(unittest.TestCase):
    ...
    def test_DiffFD_sin(self):
        f = fx.fsin
        x = 2
        dr, r = pyadi.DiffFD(f, x, verbose=self.verbose, dump=self.dump)
    ...
    def test_DiffFD_partial(self):
        f = fxyz.f4
        x,y,z = 1,2,3
        dr, r = pyadi.DiffFD(f, x, y, z)
        dr_x, r = pyadi.DiffFD(f, x, y, z, active='x')
        dr_y, r = pyadi.DiffFD(f, x, y, z, active='y')
        dr_z, r = pyadi.DiffFD(f, x, y, z, active='z')
        self.assertTrue(almostEq(dr[0], dr_x))
        self.assertTrue(almostEq(dr[1], dr_y))
        self.assertTrue(almostEq(dr[2], dr_z))

        dr_y2, r = pyadi.DiffFD(f, x, y, z, seed=[ [0,1,0] ])
        self.assertTrue(almostEq(dr_y2, dr_y))

        with self.assertRaises(ValueError):
            dr_y2, r = pyadi.DiffFD(f, x, y, z, seed=[ [0,1] ])

        dr_z, r = pyadi.DiffFD(f, x, y, z, active='fz2')
    ...
    def test_DiffFD_partial3(self):
        ...
        x,y,z = 1,2,3
        dr, r = pyadi.DiffFD(f, x, y, z)
        dr_x, r = pyadi.DiffFD(f, x, y, z, active=[0])
        dr_y, r = pyadi.DiffFD(f, x, y, z, active=[1])
        dr_z, r = pyadi.DiffFD(f, x, y, z, active=[2])
        self.assertTrue(almostEq(dr[0], dr_x))
        self.assertTrue(almostEq(dr[1], dr_y))
        self.assertTrue(almostEq(dr[2], dr_z))
    ...
    def test_DiffFD_partial4(self):
        ...
        dr_dx2, r = pyadi.DiffFD(f, x, y, z, seed=[ dx ])
        #print('r2', dr_dx2, dr)
    ...
    def test_DiffFD_partial6(self):
        ...
        dr, r = pyadi.DiffFDNP(fnp, v)

        dft = lambda t: fnp(v + t*dx)
        self.assertTrue(almostEq(dft(0.0), r))

        dr_dx, r = pyadi.DiffFDNP(dft, np.array([0.0]))

        dr_dx2, r = pyadi.DiffFDNP(fnp, v, seed=[ dx ])
    ...
    def test_DiffFD_partial7(self):
        ...
        dr_dx, r = pyadi.DiffFDNP(dft, np.array([0.0]))
        dr_dx_ad, r = pyadi.DiffFor(dft, np.array([0.0]))
